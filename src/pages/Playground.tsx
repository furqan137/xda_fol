...
const Playground = () => {
  ...
  // Show all projects regardless of active filter
  const filteredProjects = playgroundProjects;
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2 rounded-full text-white text-sm font-medium transition ${
                  activeFilter === cat.id ? `${cat.color} shadow-md` : "bg-secondary text-text-secondary hover:text-white"
                }`}
              >
